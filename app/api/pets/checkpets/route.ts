...
function keyOf(p: { petname: string; variant: PetVariant; fly: boolean; ride: boolean }) {
  return `${p.petname.toLowerCase()}|${p.variant}|${p.fly}|${p.ride}`;
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingSet = new Set(
      existing.map((p) => `${p.name.toLowerCase()}|${p.variant}|${p.fly}|${p.ride}`)
    );
    ...
    const created = await Promise.all(
      missing.map(async (p) => {
        const res = await fetch(`${origin}/api/pets/create`, {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            name: p.petname.toLowerCase(),
            value: defaultValue, // required by schema
            variant: p.variant,
            fly: p.fly,
            ride: p.ride,
          }),
        ...
        // If already exists (race condition), treat as ok
        if (res.status === 409) {
          return { status: "exists", input: p };
        }

        if (!res.ok) {
          const details = await res.text().catch(() => "");
          return { status: "failed", input: p, code: res.status, details };
        }

        const data = await res.json();
        return { status: "created", petType: data.petType };
      })
    );
